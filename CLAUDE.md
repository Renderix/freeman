...
- `wakeword/`: Porcupine wake word detector (always-on, never muted) — detects wake/mute/stop keywords
- Self-echo prevention: VAD + transcriber are muted during TTS playback via `Muter` interface

### Layer 2: Conversation (`internal/conv/`)
- `session.go`: Long-lived event loop routing between wakeword detector, transcriber, speaker, task manager, and conv-sidecar
...
**Voice Call:** Mic → Porcupine (wake word) → VAD → Whisper STT → ConvSession → conv-sidecar (LLM) → SentenceBuffer → Engine (Kokoro PCM) → Speaker
...
`config.yaml` controls: server port, model paths, TTS defaults, conversation model settings, audio devices, STT/VAD parameters, persona settings, transcript logging.

Key config sections: `server`, `model` (Kokoro paths), `tts` (voice/speed/buffer), `freeman.pm` (conversation model), `freeman.worker` (task agent models), `freeman.audio`, `freeman.stt` (Whisper + VAD). Top-level `persona:` key configures the voice persona (name, greeting, traits, rules, Porcupine wake/mute/stop keyword paths, sensitivities).
